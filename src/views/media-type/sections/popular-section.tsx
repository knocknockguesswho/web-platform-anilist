...
const PopularSection = (props: IProps) => {
  const { loading, error, data } = useQuery<IMediaListResponse, IMediaListQueryParams>(getMediaList, {
    variables: {
      page: 1,
      perPage: 5,
      sort: MEDIA_SORT_PARAMS.POPULARITY_DESC,
      type: props.selectedMediaType,
    },
  });
  return (
    <section id='home-media-list-popular'>
      <MediaListHorizontalTemplate
        title='Popular'
        loading={loading}
        error={error}
        data={data?.data.media}
        viewAllHref={`/${typeNameMap[props.selectedMediaType]}/list/${sortNameMap[MEDIA_SORT_PARAMS.POPULARITY_DESC]}`}
